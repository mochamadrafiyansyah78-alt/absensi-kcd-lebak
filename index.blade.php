<!DOCTYPE html>
<html lang="id">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Absensi - KCD Pendidikan Kab. Lebak</title>
    <script src="https://cdn.tailwindcss.com"></script>
    <link rel="stylesheet" href="https://unpkg.com/leaflet@1.9.4/dist/leaflet.css" />
    <script src="https://unpkg.com/leaflet@1.9.4/dist/leaflet.js"></script>
</head>
<body class="bg-gray-100 min-h-screen py-6">

    <div class="max-w-md mx-auto bg-white rounded-lg shadow-md overflow-hidden">

        {{-- Header --}}
        <div class="bg-blue-700 text-white p-4 text-center">
            <h1 class="font-bold text-lg leading-tight">KCD Pendidikan Kab. Lebak</h1>
            <p class="text-xs opacity-90">KCD Dinas Pendidikan Kabupaten Lebak</p>
            <p class="mt-2 text-sm font-mono" id="jam-server">--:--:--</p>
        </div>

        <div class="p-5">

            {{-- Pesan sukses / error --}}
            @if (session('success'))
                <div class="mb-4 p-3 bg-green-100 text-green-700 rounded text-sm">
                    {{ session('success') }}
                </div>
            @endif

            @if (session('error'))
                <div class="mb-4 p-3 bg-red-100 text-red-700 rounded text-sm">
                    {{ session('error') }}
                </div>
            @endif

            @if ($errors->any())
                <div class="mb-4 p-3 bg-red-100 text-red-700 rounded text-sm">
                    <ul class="list-disc list-inside">
                        @foreach ($errors->all() as $error)
                            <li>{{ $error }}</li>
                        @endforeach
                    </ul>
                </div>
            @endif

            <form action="{{ route('absen.store') }}" method="POST" id="form-absen">
                @csrf

                {{-- Dropdown Nama Pegawai --}}
                <div class="mb-3">
                    <label class="block text-sm font-medium text-gray-700 mb-1">Nama Pegawai</label>
                    <select name="karyawan_id" id="karyawan_id" class="w-full border rounded px-3 py-2" required>
                        <option value="">-- Pilih Nama --</option>
                        @foreach ($karyawans as $k)
                            <option
                                value="{{ $k->id }}"
                                data-nip="{{ $k->nip }}"
                                data-jabatan="{{ $k->jabatan }}"
                                {{ old('karyawan_id') == $k->id ? 'selected' : '' }}
                            >
                                {{ $k->nama }}
                            </option>
                        @endforeach
                    </select>
                </div>

                {{-- NIP (readonly) --}}
                <div class="mb-3">
                    <label class="block text-sm font-medium text-gray-700 mb-1">NIP</label>
                    <input type="text" id="nip" class="w-full border rounded px-3 py-2 bg-gray-100" readonly>
                </div>

                {{-- Jabatan (readonly) --}}
                <div class="mb-3">
                    <label class="block text-sm font-medium text-gray-700 mb-1">Jabatan</label>
                    <input type="text" id="jabatan" class="w-full border rounded px-3 py-2 bg-gray-100" readonly>
                </div>

                {{-- Status Kehadiran --}}
                <div class="mb-3">
                    <label class="block text-sm font-medium text-gray-700 mb-1">Status Kehadiran</label>
                    <div class="flex gap-4">
                        <label class="flex items-center gap-1">
                            <input type="radio" name="status_kehadiran" value="hadir" checked> Hadir
                        </label>
                        <label class="flex items-center gap-1">
                            <input type="radio" name="status_kehadiran" value="izin"> Izin
                        </label>
                        <label class="flex items-center gap-1">
                            <input type="radio" name="status_kehadiran" value="sakit"> Sakit
                        </label>
                    </div>
                </div>

                {{-- Tipe Absensi (hanya tampil & relevan jika Hadir) --}}
                <div class="mb-4" id="wrap-tipe-absensi">
                    <label class="block text-sm font-medium text-gray-700 mb-1">Tipe Absensi</label>
                    <div class="flex gap-4">
                        <label class="flex items-center gap-1">
                            <input type="radio" name="tipe_absensi" value="masuk" checked> Masuk (Max 07:45)
                        </label>
                        <label class="flex items-center gap-1">
                            <input type="radio" name="tipe_absensi" value="pulang"> Pulang
                        </label>
                    </div>
                </div>

                {{-- Modul Kamera (wajib untuk semua status, tanpa opsi galeri) --}}
                <div class="mb-4">
                    <label class="block text-sm font-medium text-gray-700 mb-1">Bukti Foto (Wajib difoto langsung)</label>

                    <button type="button" id="btn-buka-kamera" class="w-full bg-gray-700 text-white text-sm py-2 rounded mb-2">
                        Buka Kamera
                    </button>

                    {{-- Video live preview (muncul saat kamera dibuka) --}}
                    <video id="video-preview" class="w-full rounded mb-2 hidden" autoplay playsinline></video>
                    <button type="button" id="btn-ambil-foto" class="w-full bg-blue-600 text-white text-sm py-2 rounded mb-2 hidden">
                        Ambil Foto
                    </button>

                    {{-- Canvas tersembunyi untuk proses capture --}}
                    <canvas id="canvas-foto" class="hidden"></canvas>

                    {{-- Preview hasil foto --}}
                    <img id="preview-foto" class="w-full rounded mb-2 hidden" alt="Preview Foto">
                    <button type="button" id="btn-ulangi-foto" class="w-full bg-yellow-600 text-white text-sm py-2 rounded mb-2 hidden">
                        Ambil Ulang
                    </button>

                    <input type="hidden" name="foto_base64" id="foto_base64">

                    <p id="status-foto" class="text-xs text-gray-500">Belum ada foto diambil. Foto wajib diambil langsung dari kamera.</p>
                </div>

                {{-- Modul Geolokasi (otomatis + marker dapat digeser/dipilih manual) --}}
                <div class="mb-4">
                    <label class="block text-sm font-medium text-gray-700 mb-1">Lokasi GPS</label>
                    <div id="map" class="w-full h-48 rounded border mb-2"></div>
                    <p class="text-xs text-gray-400 mb-1">Geser penanda pada peta jika titik GPS kurang presisi.</p>
                    <p id="status-gps" class="text-xs text-gray-500">Mendeteksi lokasi...</p>
                    <input type="hidden" name="latitude" id="latitude">
                    <input type="hidden" name="longitude" id="longitude">
                </div>

                <button type="submit" class="w-full bg-blue-700 text-white font-bold py-3 rounded hover:bg-blue-800">
                    KIRIM ABSENSI SEKARANG
                </button>
            </form>
        </div>
    </div>

    <script>
        // ================= AUTO-SYNC NIP & JABATAN =================
        const selectKaryawan = document.getElementById('karyawan_id');
        const inputNip = document.getElementById('nip');
        const inputJabatan = document.getElementById('jabatan');

        function syncKaryawan() {
            const selected = selectKaryawan.options[selectKaryawan.selectedIndex];
            inputNip.value = selected.dataset.nip || '';
            inputJabatan.value = selected.dataset.jabatan || '';
        }
        selectKaryawan.addEventListener('change', syncKaryawan);
        syncKaryawan();

        // ================= TOGGLE TIPE ABSENSI =================
        // Catatan: Kamera & GPS TIDAK ikut disembunyikan di sini karena wajib untuk semua status.
        const radiosStatus = document.querySelectorAll('input[name="status_kehadiran"]');
        const wrapTipe = document.getElementById('wrap-tipe-absensi');

        function toggleTipeAbsensi() {
            const checked = document.querySelector('input[name="status_kehadiran"]:checked').value;
            wrapTipe.style.display = (checked === 'hadir') ? 'block' : 'none';
        }
        radiosStatus.forEach(r => r.addEventListener('change', toggleTipeAbsensi));
        toggleTipeAbsensi();

        // ================= JAM SERVER REAL-TIME =================
        let jamServer = new Date("{{ now('Asia/Jakarta')->toIso8601String() }}");
        function updateJam() {
            jamServer.setSeconds(jamServer.getSeconds() + 1);
            document.getElementById('jam-server').innerText = jamServer.toLocaleTimeString('id-ID', { hour12: false }) + ' WIB';
        }
        setInterval(updateJam, 1000);
        updateJam();

        // ================= MODUL KAMERA (Wajib, Tanpa Galeri) =================
        const btnBukaKamera = document.getElementById('btn-buka-kamera');
        const btnAmbilFoto = document.getElementById('btn-ambil-foto');
        const btnUlangiFoto = document.getElementById('btn-ulangi-foto');
        const videoPreview = document.getElementById('video-preview');
        const canvasFoto = document.getElementById('canvas-foto');
        const previewFoto = document.getElementById('preview-foto');
        const inputFotoBase64 = document.getElementById('foto_base64');
        const statusFoto = document.getElementById('status-foto');

        let streamKamera = null;

        async function bukaKamera() {
            try {
                streamKamera = await navigator.mediaDevices.getUserMedia({
                    video: { facingMode: 'environment' }
                });
                videoPreview.srcObject = streamKamera;
                videoPreview.classList.remove('hidden');
                btnAmbilFoto.classList.remove('hidden');
                btnBukaKamera.classList.add('hidden');
                previewFoto.classList.add('hidden');
                btnUlangiFoto.classList.add('hidden');
                statusFoto.innerText = 'Kamera aktif. Arahkan lalu tekan "Ambil Foto".';
            } catch (err) {
                statusFoto.innerText = 'Gagal mengakses kamera: ' + err.message + '. Pastikan izin kamera diaktifkan.';
            }
        }

        btnBukaKamera.addEventListener('click', bukaKamera);

        btnAmbilFoto.addEventListener('click', () => {
            const context = canvasFoto.getContext('2d');
            canvasFoto.width = videoPreview.videoWidth;
            canvasFoto.height = videoPreview.videoHeight;
            context.drawImage(videoPreview, 0, 0, canvasFoto.width, canvasFoto.height);

            const base64 = canvasFoto.toDataURL('image/jpeg', 0.8);
            inputFotoBase64.value = base64;

            previewFoto.src = base64;
            previewFoto.classList.remove('hidden');
            btnUlangiFoto.classList.remove('hidden');
            videoPreview.classList.add('hidden');
            btnAmbilFoto.classList.add('hidden');
            statusFoto.innerText = 'Foto berhasil diambil.';

            if (streamKamera) {
                streamKamera.getTracks().forEach(track => track.stop());
            }
        });

        btnUlangiFoto.addEventListener('click', () => {
            inputFotoBase64.value = '';
            previewFoto.classList.add('hidden');
            btnUlangiFoto.classList.add('hidden');
            btnBukaKamera.classList.remove('hidden');
            statusFoto.innerText = 'Silakan ambil ulang foto.';
            bukaKamera();
        });

        // ================= MODUL GEOLOKASI (Marker Dapat Digeser/Dipilih) =================
        const statusGps = document.getElementById('status-gps');
        const inputLat = document.getElementById('latitude');
        const inputLng = document.getElementById('longitude');
        let peta = null;
        let markerPeta = null;

        function setLokasi(lat, lng, sumber) {
            inputLat.value = lat;
            inputLng.value = lng;
            statusGps.innerText = 'Lokasi (' + sumber + '): ' + lat.toFixed(6) + ', ' + lng.toFixed(6);
        }

        function initPeta(lat, lng) {
            peta = L.map('map').setView([lat, lng], 17);

            L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png', {
                attribution: '&copy; OpenStreetMap contributors'
            }).addTo(peta);

            markerPeta = L.marker([lat, lng], { draggable: true }).addTo(peta)
                .bindPopup('Geser penanda untuk koreksi lokasi')
                .openPopup();

            // Update koordinat saat marker digeser manual
            markerPeta.on('dragend', function (e) {
                const pos = e.target.getLatLng();
                setLokasi(pos.lat, pos.lng, 'dipilih manual');
            });

            // Klik pada peta juga memindahkan marker
            peta.on('click', function (e) {
                markerPeta.setLatLng(e.latlng);
                setLokasi(e.latlng.lat, e.latlng.lng, 'dipilih manual');
            });
        }

        function ambilLokasi() {
            if (!navigator.geolocation) {
                statusGps.innerText = 'Browser tidak mendukung Geolocation API.';
                return;
            }

            navigator.geolocation.getCurrentPosition(
                (position) => {
                    const lat = position.coords.latitude;
                    const lng = position.coords.longitude;

                    if (!peta) {
                        initPeta(lat, lng);
                    } else {
                        peta.setView([lat, lng], 17);
                        markerPeta.setLatLng([lat, lng]);
                    }

                    setLokasi(lat, lng, 'GPS otomatis');
                },
                (error) => {
                    statusGps.innerText = 'Gagal mendapatkan lokasi otomatis: ' + error.message + '. Peta default Kabupaten Lebak, silakan klik untuk memilih lokasi.';
                    // Fallback: pusatkan peta ke wilayah Kabupaten Lebak jika GPS gagal/ditolak
                    if (!peta) {
                        initPeta(-6.5587, 106.2467);
                    }
                },
                { enableHighAccuracy: true, timeout: 10000 }
            );
        }

        ambilLokasi();

        // ================= VALIDASI SEBELUM SUBMIT (berlaku untuk SEMUA status) =================
        document.getElementById('form-absen').addEventListener('submit', function (e) {
            if (!inputFotoBase64.value) {
                e.preventDefault();
                alert('Foto bukti wajib diambil langsung dari kamera.');
                return;
            }
            if (!inputLat.value || !inputLng.value) {
                e.preventDefault();
                alert('Lokasi GPS belum ditentukan. Mohon tunggu deteksi otomatis atau klik peta untuk memilih lokasi.');
                return;
            }
        });
    </script>

</body>
</html>